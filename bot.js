...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['Bhelp','Type Bhelp','BasHa'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith(prefix + 'Bhelp')) { /// This is The DMS Code Send The Help In DMS // Code By NotGucci
    let pages = [`**
        ***__General orders__***
**
...
var prefix = "Your Prefix"   //اذا كنت معرف البريفكس من قبل احذف هاد السطر
...
if(commandking === "setroleget") {
    ...
    monster.channel.send(`Successfully applied RoleGet to \`${newKing}\``)
...
client.on('message', monster => {
let commandking = monster.content.split(" ")[0].slice(prefix.length);
if(commandking === "roleget"){
  if(monster.author.bot || monster.channel.type == 'dm') return;
  let rolegetid = monster.guild.roles.find(mk => mk.name === monsterking[monster.guild.id].role)
    if(!rolegetid) return monster.channel.send("Sorry But there's no RoleGet Command on our system activated!!")
